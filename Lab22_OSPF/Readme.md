...
Задача настроить динамическую маршрутизацию с использованием OSPF, чтобы сети NetXX имели сетевую связность.

#### 1.2 Provisioning

Выполняем предварительную подготовку роутеров 1-3 с установкой необходимых общих утилит для работы с сетью. Используем Ansible playbook provision.yaml. Все playbookи находятся в отдельной папке ansible/playbooks.
...
* активировали OSPF
* сделали анонс сетей 10.0.0.x/30, а также 172.10.10.0/24 в одной зоне area 0
* задали router-id 1.1.1.1 
* задали соседей

На роутерах 2-3 выполняем аналогичные действия.
...
Для динамического формирования frr.conf будем использовать шаблонизатор jinj2.
...
чтобы восстановить симметричный роутинг увеличим стоимость интерфейса в сторону router1 на router3:
...
В трафике на router2 видим, что теперь весь трафик проходит одним путем(видны как request, так и reply.)
