...
Проверяем доступные блочные устройства в системе:
...
Создаем четыре пула тип "mirror" (по типу RAID1):
...
Проверяем, что 4 пула созданы, объем каждого пула 1Gb (блочные устройства по 1GB, для RAID1 два диска дают 1Gb доступного пр-ва):
`````
...
Настраиваем для каждого из пулов отдельный алгоритм сжатия, который будет использоваться при записи данных в файловую систему:
`````
...
Смотрим на кол-во занятого пространства в каждом из пулов:
...
Видим, что наименьший занятый объем в 3ем пуле, где используется алгоритм сжатия  gzip-9.
...
Проверим степень компрессии для каждого из пулов по данным системы:
````
...
Видим, что 3-ий пул имеет наивысшую степень равную 3.65x. При этом алгоритм zle в данном случае не дал никакого сжатия данных и объем данных не уменьшился при записи.
...
Таким образом, при выборе алгоритма сжатия необходимо учитывать актуальный тип данных, а также требования к скорости работы.  
...
`````
Видим, что пул успешно симпортировался, имеет тип RAID1 (в контексте zfs RAID1 - mirror-0).
...
Проверяем результат копирования файловой системы и монтирования:
